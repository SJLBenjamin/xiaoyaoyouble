        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    Reissue projectBean = (Reissue) objList.get(j);
                    List<String> list = new ArrayList<>();
                    list.add(""+(projectBean.getDetailData()));
                    list.add(""+(projectBean.getData()));
                    list.add(""+(projectBean.getNumber()));
                    list.add(""+(projectBean.getBlename()));
...
//                    holder.context.setText(mAll.get(position).getData());
    ...
    public static <T> void writeObjListToExcelToReceiveRom(List<T> objList, String fileName, Context c) {
        if (objList != null && objList.size() > 0) {
            WritableWorkbook writebook = null;
            InputStream in = null;
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ReissueToRom projectBean = (ReissueToRom) objList.get(j);
                    List<String> list = new ArrayList<>();
                    list.add(""+(projectBean.getDetailData()));
                    list.add(""+(projectBean.getData()));
                    list.add(""+(projectBean.getNumber()));
